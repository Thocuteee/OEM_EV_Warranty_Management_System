...
public class PartController {
    ...
    public ResponseEntity<PartResponse> getPartById(@PathVariable Long id) {
        ...
        return partOpt.map(part -> ResponseEntity.ok(toResponse(part))).orElse(ResponseEntity.notFound().build());
    }
    ...
    // Cập nhật linh kiện
    @PutMapping("/{id}")
    public ResponseEntity<?> updatePart(@PathVariable Long id, @Valid @RequestBody PartRequest request) {
        
        // 1. Kiểm tra tồn tại trước
